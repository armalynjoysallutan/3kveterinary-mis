...
$stockStatusInterpretation =
    "The Stock Status chart presents the current "
    . "availability of active inventory items.";

if ($totalItems > 0) {

    $inStockPercentage =
        ($inStock / $totalItems) * 100;

    $reorderPercentage =
        ($reorderSoon / $totalItems) * 100;

    $outOfStockPercentage =
        ($outOfStock / $totalItems) * 100;

    $stockStatusInterpretation .=
        " Of the "
        . number_format($totalItems)
        . " active inventory item(s), "
        . number_format($inStockPercentage, 1)
        . "% are currently in stock, "
        . number_format($reorderPercentage, 1)
        . "% are at or below their reorder level, and "
        . number_format($outOfStockPercentage, 1)
        . "% are out of stock.";
} else {

    $stockStatusInterpretation .=
        " No active inventory items are currently recorded.";
}
...
$categoryInterpretation =
    "The Items by Category chart shows the number of "
    . "active inventory items under each category.";

if (!empty($categoryChartLabels)) {

    $topCategoryIndex = 0;

    foreach ($categoryChartValues as $index => $value) {

        if (
            $value >
            $categoryChartValues[$topCategoryIndex]
        ) {
    ...
    $topCategory =
        $categoryChartLabels[$topCategoryIndex];

    $topCategoryCount =
        $categoryChartValues[$topCategoryIndex];

    $categoryInterpretation .=
        " The category with the highest number of active "
        . "items is "
        . $topCategory
        . " with "
        . number_format($topCategoryCount)
        . " item(s).";
} else {

    $categoryInterpretation .=
        " No active inventory categories are currently recorded.";
}
...
$categoryQuantityInterpretation =
    "The Stock Quantity by Category chart presents the "
    . "current quantity of inventory available in each category.";

if (!empty($categoryStockLabels)) {

    $highestQuantityIndex = 0;

    foreach ($categoryStockValues as $index => $value) {

        if (
            $value >
            $categoryStockValues[$highestQuantityIndex]
        ) {
    ...
    $highestQuantityCategory =
        $categoryStockLabels[$highestQuantityIndex];
    ...
    $categoryQuantityInterpretation .=
        " "
        . $highestQuantityCategory
        . " has the highest recorded stock quantity with "
        . number_format($highestQuantity, 0)
        . " unit(s).";
} else {

    $categoryQuantityInterpretation .=
        " No stock quantities are currently recorded.";
}
...
$inventoryValueInterpretation =
    "The Inventory Value section estimates the financial "
    . "value of the currently recorded active inventory based "
    . "on stock quantity, unit cost, and retail price.";
...
if ($totalQuantity > 0) {
    ...
    $inventoryValueInterpretation .=
        ...
        . " unit(s), with an estimated acquisition cost of "
        . "₱"
        . number_format($totalInventoryCost, 2)
        . " and an estimated retail value of "
        . "₱"
    ...
    $inventoryValueInterpretation .=
        " No inventory quantity is currently recorded.";
}
            ...
            <div class="inventory-report-chart-grid">
                ...
                <div class="report-card">
                    ...
                    <div class="print-interpretation">
                        ...
                        <p>
                            <?= htmlspecialchars($stockStatusInterpretation) ?>
                        </p>
                    </div>
                ...
                <div class="report-card">
                    ...
                    <div class="print-interpretation">
                        ...
                        <p>
                            <?= htmlspecialchars($categoryInterpretation) ?>
                        </p>
            ...
            <div class="inventory-report-chart-grid">
                ...
                <div class="report-card">
                    ...
                    <div class="print-interpretation">
                        ...
                        <p>
                            <?= htmlspecialchars($categoryQuantityInterpretation) ?>
                        </p>
                    </div>
                ...
                <div class="report-card">
                    ...
                    <div class="print-interpretation">
                        <h3>Interpretation</h3>
                        <p>
                            <?= htmlspecialchars($inventoryValueInterpretation) ?>
                        </p>
                    </div>
