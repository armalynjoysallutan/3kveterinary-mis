...
const filterMenu = document.getElementById("inventoryFilterMenu");
const filterLabel = document.getElementById("inventoryFilterLabel");
...
document.addEventListener("click", function (event) {
    ...
    if (actionType === "archive") {
        const itemName = action.dataset.name || "Inventory Item";
        openInventoryArchiveModal(itemId, itemName);
        return;
    }
    ...
    <div class="inventory-form-group">
        <label for="stockInBatchNumber">
            Batch Number <span>*</span>
        </label>

        <input
            type="text"
            id="stockInBatchNumber"
            name="batch_number"
            readonly
        ...
        <small
            style="display:block;margin-top:6px;color:#718096;"
        >
    ...
    <div class="inventory-form-group">
        <label for="stockInExpiration">
            Expiration Date
        </label>

        <input
            type="date"
            id="stockInExpiration"
            name="expiration_date"
        >
    </div>

    <div class="inventory-form-group">
        <label for="stockInDateReceived">
            Date Received <span>*</span>
        </label>

        <input
            type="date"
            id="stockInDateReceived"
            name="date_received"
            required
        >
    </div>
...
<div class="inventory-stock-in-divider">
...
<div class="inventory-form-grid">
    ...
    <div class="inventory-form-group">
        <label for="stockInPackagingType">
            Packaging Type <span>*</span>
        </label>

        <select
            id="stockInPackagingType"
            name="packaging_type"
            required
        >
    ...
    <div class="inventory-form-group">
        <label for="stockInPackageQuantity">
            No. of Packages <span>*</span>
        </label>

        <input
            type="number"
            id="stockInPackageQuantity"
    ...
    <div class="inventory-form-group">
        <label for="stockInUnitsPerPackage">
            Units per Package <span>*</span>
        </label>

        <input
            type="number"
            id="stockInUnitsPerPackage"
    ...
    <div class="inventory-form-group">
        ...
        ></small>
                      ...
                      <div class="inventory-form-note">
                        ...
                        <span>
                           A new batch will be created for every new stock delivery.
                           Packaging quantity will be converted into the item's inventory unit.
        ...
        const packageQuantityInput =
    modal.querySelector("#stockInPackageQuantity");

const unitsPerPackageInput =
    modal.querySelector("#stockInUnitsPerPackage");

const actualQuantityInput =
    modal.querySelector("#stockInQuantity");

const quantityUnit =
    modal.querySelector("#stockInQuantityUnit");

function updateStockInActualQuantity() {
    const packageQuantity =
        parseFloat(packageQuantityInput?.value || "0");

    const unitsPerPackage =
        parseFloat(unitsPerPackageInput?.value || "0");
...
packageQuantityInput?.addEventListener(
    "input",
    updateStockInActualQuantity
);

unitsPerPackageInput?.addEventListener(
    "input",
    updateStockInActualQuantity
);
...
if (unitsPerPackageInput) {
    unitsPerPackageInput.value = "";
        ...
        // Get the next auto-generated batch number
const batchNumberInput =
    modal.querySelector("#stockInBatchNumber");

if (batchNumberInput) {
    batchNumberInput.value = "Generating...";

    fetch(`get_next_batch_number.php?item_id=${encodeURIComponent(itemId)}`)
    .then(async response => {

        const responseText = await response.text();

        console.log("Batch Number Response:", responseText);
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="inventory-success-modal"
                ...
                <button type="button" id="inventorySuccessOk">
                    OK
                </button>
...
let inventoryArchiveItemId = null;    


/* =========================================================
   OPEN ARCHIVE MODAL FOR SINGLE ITEM
   ========================================================= */

function openInventoryArchiveModal(itemId, itemName) {

    inventoryArchiveItemId = itemId;
    ...
    if (inventoryArchiveError) {
        ...
        inventoryArchiveError.style.display = "none";
    }

    if (inventoryArchiveReason) {
        inventoryArchiveReason.value = "";
    }
        ...
        async function () {
            ...
            if (reason === "") {
                ...
                if (inventoryArchiveError) {
                    ...
                    inventoryArchiveError.textContent =
                        "Please select a reason for archiving this inventory item.";
            ...
            try {
                ...
                showInventorySuccessModal(
                    "Archive Successful",
                    result.message || "Inventory item has been archived successfully."
                );    

                inventoryArchiveItemId = null;
...
function renderViewBatches(batches) {
    ...
    Number.isInteger(Number(batch.quantity_received ?? 0))
        ? Number(batch.quantity_received ?? 0).toString()
...
const remainingQuantity =
    Number.isInteger(Number(batch.quantity ?? 0))
        ? Number(batch.quantity ?? 0).toString()
        ...
        const dateReceived =
            batch.date_received_display ||
            batch.date_received ||
            "—";

        const expiration =
            batch.expiration_display ||
    ...
    wrapper.innerHTML = `
        ...
        <div class="inventory-view-batch-scroll">
